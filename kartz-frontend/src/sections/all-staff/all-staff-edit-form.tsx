...
import { requestEditAllStaff } from 'src/store/all-staff/allStaffThunk';
...
export const NewStaffSchema = zod.object({
  ...
  countryCode: zod.string().min(1, { message: 'Select a country!' }),
...
type Props = {
  currentData: IAllStaffListTypes;
};

export function AllStaffEditForm({ currentData }: Props) {
  const router = useRouter();
  const dispatch = useAppDispatch();

  const { data, loading } = useAppSelector((state) => state.allstaff.edit);
  ...
  const defaultValues = useMemo(
    () => ({
      ...
      permissionId: currentData?.permissionId || '',
    }),
    [currentData]
  );
  ...
  useEffect(() => {
    if (data?.staffCreated || data?.staffUpdated) {
  ...
  const renderDetails = (
    <Card>
      ...
      <Stack spacing={3} sx={{ p: 3, pt: 0 }}>
        ...
        <Field.Phone country="OM" name="phone" label="Phone number" />
        <Field.Select
          name="permissionId"
          label="Role"
        ...
        />
        {/* <Field.CountrySelect name="okok" label="Country" onChange={(e, countryName) => {
