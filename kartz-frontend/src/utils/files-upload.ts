...
import { API_METHODS, ENDPOINT_REQUEST_FILE_UPLOAD, makeNetworkCall } from 'src/network';
...
export async function uploadFile(props: fileUploadType): Promise<string | boolean> {
  ...
  try {
    ...
    const urlForUpload = `${ENDPOINT_REQUEST_FILE_UPLOAD}?${urlParamsForUpload.toString()}`;
